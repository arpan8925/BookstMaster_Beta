...
import secrets
from managerdashboard.models import PaymentMethod, Transactions, Order
from decimal import Decimal
from django.db import connection
from django.http import JsonResponse
from django.db import connection
from .forms import AddFundsForm, OrderForm
from managerdashboard.models import ServiceCategory, Service
...
def calculate_fee(payment_method, amount):
    if payment_method.fee_type == "percentage":
        ...
        return (
            amount * (payment_method.fee_percentage / Decimal("100"))
...
def search_services(request):
    ...
    return JsonResponse(
        {"results": []}
    )  # Return empty if no query is provided or no matches found
...
def dashboard(request):
    ...
    for service in services:
        cat_id = str(service["category_id"])
        cat_name = service["category_name"]
        if cat_id not in categories:
            categories[cat_id] = {"name": cat_name, "services": []}
        service_data = {
            "id": service["id"],
            "name": service["name"],
            "desc": service["desc"],
        ...
        categories[cat_id]["services"].append(service_data)

    # Handle form submission
    if request.method == "POST":
        logger.debug("Form submission received")
        form = OrderForm(request.POST, categories=categories)
        if form.is_valid():
            ...
            try:
                ...
                service = Service.objects.filter(
                    id=service_id, cate_id=category_id, status=1
                ).first()
                if not service:
                ...
                service = Service.objects.get(id=service_id, cate_id=category_id)
                ...
                return redirect("user_dashboard")
            except (ServiceCategory.DoesNotExist, Service.DoesNotExist):
                messages.error(request, "Invalid service selection")
        ...
        form = OrderForm(categories=categories)
...
def profile(request):
    if request.method == "POST":
        ...
        return render(
            ...
            {"active_tab": "profile", "show_success_alert": True},
        )
...
def profile_more(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "user_dashboard/profile.html",
            {"active_tab": "profile", "show_additional_success_alert": True},
        )
...
def order_log(request):
    ...
    # Existing order filtering and processing logic
    ...
    if search:
        orders = orders.filter(
            models.Q(service_id__name__icontains=search)
    ...
    # Pagination
    paginator = Paginator(orders.order_by("-created"), 10)
...
def api_documentation(request):
    ...
    context = {"active_tab": "api", "api_data": api_data}
...
def add_funds(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        if not terms:
            ...
            return render(
                request,
                "user_dashboard/AddFund.html",
                {"form": form, "payment_methods": payment_methods},
            )
        ...
        if payment_method.type == "manual":
            ...
            messages.success(
                request,
                f"Transaction initiated successfully. Your transaction ID is {transaction.transaction_id}. Please follow the instructions to complete the payment.",
            )
            return redirect(
...
def tickets(request):
    ...
    context = {"tickets": tickets_page, "active_tab": "tickets"}
    ...
    if request.method == "POST":
        ...
        if subject and content:
            ...
            TicketMessage.objects.create(
                ticket=ticket, user=request.user, message=content
            )
...
def view_ticket(request, ticket_id):
    ...
    messages = ticket.messages.all().order_by(
        "created"
    )  # Changed from replies to messages

    context = {"ticket": ticket, "messages": messages, "active_tab": "tickets"}
...
def reply_ticket(request, ticket_id):
    ...
    if message:
        TicketMessage.objects.create(ticket=ticket, user=request.user, message=message)
    ...
    if request.method == "POST":
        try:
            ...
            if method.fee_type == "percentage":
                ...
                fee = (
                    amount * (method.fee_percentage / Decimal("100"))
                ) + method.fee_fixed
...
def transaction_detail(request, transaction_id):
    transaction = get_object_or_404(
        Transactions, transaction_id=transaction_id, uid=request.user.uid
    )
    return render(
        request, "user_dashboard/transaction_detail.html", {"transaction": transaction}
    )
...
def ticket_detail(request, pk):
    ...
    if request.method == "POST":
        ...
        if message:
            TicketMessage.objects.create(
                ticket=ticket, user=request.user, message=message
            )
    ...
    return render(
        request,
        "user_dashboard/tickets/ticket_detail.html",
        {"ticket": ticket, "replies": ticket.messages.all()},
    )
